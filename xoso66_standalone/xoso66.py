...
def worker_session_health(interval_sec: int, *, quiet: bool = False) -> None:
    """Định kỳ probe session — chỉ ensure_session khi yếu và không bị CF rate limit."""
    from xoso66_accounts_db import username_for_log
    from xoso66_cf import CfRateLimitError, cf_rate_limit_remaining, is_cf_rate_limited
    from xoso66_proxy import resolve_proxy
    from xoso66_session import ensure_session, load_sessions, session_health

    ensure_cooldown = int(os.environ.get("XOSO66_WORKER_ENSURE_COOLDOWN_SEC", "600"))
    last_ensure: dict[str, float] = {}
            ...
            for row in rows:
                ...
                try:
                    ...
                    if is_cf_rate_limited(acc):
                        rem = int(cf_rate_limit_remaining(acc))
                        if not quiet:
                            print(
                                f"[WORKER] {username_for_log(aid)}: CF rate limit — bỏ qua ~{rem}s",
                                flush=True,
                            )
                        continue
                    ...
                    since = time.time() - last_ensure.get(aid, 0.0)
                    if since < ensure_cooldown:
                        if not quiet:
                            print(
                                f"[WORKER] {username_for_log(aid)} session yếu — "
                                f"chờ cooldown ensure ({int(ensure_cooldown - since)}s)",
                                flush=True,
                            )
                        continue

                    last_ensure[aid] = time.time()
...
def _graceful_shutdown() -> None:
    ...
    if not stopping():
        ...
        print("\n[MAIN] Đang chờ workers thoát...", flush=True)
    ...
    stubborn = [t for t in _bg_threads if t.is_alive() and not t.daemon]
    if stubborn:
        names = ", ".join(t.name for t in stubborn)
        print(f"[MAIN] Thoát ngay — thread còn sống: {names}", flush=True)
        os._exit(130)
...
def main() -> int:
    ...
    main_progress("=" * 60)
    main_progress("XOSO66 — khởi động (main.py)")
    main_progress(f"  DB: {DB_PATH}")
    main_progress(f"  Config: {os.environ.get('XOSO66_CONFIG', 'xoso66_config.json')}")
    main_progress("=" * 60)
    ...
    if cfg.get("game_worker_enabled") and not str(
        ...
        main_progress("[MAIN] Chuẩn bị pool WS «Đang Chơi»…")
        try:
    ...
    startup_async = startup_async_enabled(cfg)
    ...
    try:
        if startup_async:
            ...
            main_progress(
                "[STARTUP] Kiểm tra balance + token «Đang Chơi» "
                "(API đã lên, chờ xong rồi mở worker)…"
            )
            _run_startup_checks()
    except KeyboardInterrupt:
    ...
    br = cfg.get("balance_reconcile")
    if isinstance(br, dict) and br.get("enabled", True):
        from xoso66_balance_reconcile import start_balance_reconcile_thread

        br_t = start_balance_reconcile_thread(quiet=quiet)
        if br_t is not None:
            _track_thread(br_t)
    elif not quiet:
        print("[BALANCE-RECONCILE] TẮT (balance_reconcile.enabled=false)", flush=True)

    arp = cfg.get("auto_red_packet")
    if isinstance(arp, dict) and arp.get("enabled"):
        from xoso66_auto_red_packet import start_auto_red_packet_thread

        arp_t = start_auto_red_packet_thread(quiet=quiet)
        if arp_t is not None:
            _track_thread(arp_t)
    elif not quiet:
